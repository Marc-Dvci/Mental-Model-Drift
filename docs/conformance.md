...
**How the test suite's local Bee is held to Bee's format by Bee's own code, rather than by my
...
Every integration test in this repository runs the product against `tools/bee-sim`, a local Bee
that speaks the documented `/v1` surface. That proves the product agrees with `bee-sim`. On its own
it is worth nothing, because I wrote both, and the failure mode is not a test that goes red — it is a test that stays green while both sides share the
same misconception.

That is not hypothetical here. It happened twice, and both times the tests passed:

1. **`bee-sim` wrote frames with no SSE `event:` line**, and the client read only `data:` lines
   and guessed each frame's type from its payload. Both halves agreed, and the product was guessing
   types it had been told, getting three of them wrong.
...
1. Contains `parseSSEBuffer` **copied verbatim** from `@beeai/cli` 0.7.3,
   ...
   unmodified.
2. Opens a **raw TCP socket** to `bee-sim` — not `fetch`, which would normalise the framing —
...
field; a frame written as `data:` alone is discarded with no error and no warning. So a test double
with that bug is invisible to any client that has the same bug, and the pair produce nothing
whatsoever in front of Bee's own parser.
...
Conformance is about **format**, not behaviour. `bee-sim`'s search ranking is IDF-weighted token
overlap and is not claimed to be Bee's. What is claimed is narrower and checkable: a client that
works against `bee-sim` is not working against a wire format Bee would reject.
